...
if (inputFile.filesize() == 0) {
    ...
    //read freq size
    int freqSize = inputFile.read_byte();
    ...
    for (int i = 0; i < countSymbols; i++) {
        ...
        int freq;
        if (freqSize == 1) {
            int freqByte = inputFile.read_byte();
            if (freqByte == -1) error("unexpected end of file");
            freq = freqByte;
        } else if (freqSize == 2) {
            char freqByte[2];
            int readByte = inputFile.read_bytes(freqByte, 2);
            if (readByte != 2) error("unexpecged end of file");
            uint16_t shortFreq = *reinterpret_cast<uint16_t*>(freqByte);
            freq = shortfreq;
        } else {
